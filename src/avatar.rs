 ...
 * ======
 *
 * Structures and functions for the player avatar.
 */
...
impl Plugin for AvatarPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, spawn_avatar)
            .add_systems(Update, (move_avatar, animate_avatar));
    }
}
...
/// Based on the keyboard input, move the avatar sprite.
fn move_avatar(
    key_input: Res<ButtonInput<KeyCode>>,
    time: Res<Time>,
    mut avatar: Query<(&mut Transform, &mut AnimationState), With<Avatar>>,
) {
    /* There should be exactly one avatar. */
    let Ok((mut transform, mut anim)) = avatar.single_mut() else {
        return;
    };
...
fn animate_avatar(
    time: Res<Time>,
    mut query: Query<(&mut AnimationState, &mut AnimationTimer, &mut Sprite), With<Avatar>>,
) {
    /* There should be exactly one avatar. */
